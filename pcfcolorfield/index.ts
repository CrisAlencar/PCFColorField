...
export class pcfcolorfield implements ComponentFramework.StandardControl<IInputs, IOutputs> {
	...
	public CheckMaxMinValues(): string[] {
		...
		const colorIf = this.context.parameters.colorIf.raw!;
		const backgroundcolorIf= this.context.parameters.backgroundColorIf.raw!;
		const colorElse = this.context.parameters.colorElse.raw!;
		const backgroundcolorElse = this.context.parameters.backgroundColorElse.raw!;
			...
			switch (settings) {
				...
				if(maxValue !== null && minValue !== null)
				{
					if (value <= maxValue && value >= minValue) {
						style[0] = colorIf == 'val' || colorIf == null ? 'black' : colorIf;
						style[1] = backgroundcolorIf == 'val' || backgroundcolorIf == null ? 'white' : backgroundcolorIf;
					}
					
					else {
						style[0] = colorElse == 'val' || colorElse == null ? 'black' : colorElse;
						style[1] = backgroundcolorElse == 'val' || backgroundcolorElse == null ? 'white' : backgroundcolorElse;
					}
				}
